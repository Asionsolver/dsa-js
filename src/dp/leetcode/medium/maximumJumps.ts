...
// Good Solution: O(n^2) time complexity, O(n) space complexity
...
// Optimized Solution: O(n^2) time complexity, O(n) space complexity
function maximumJumps(nums: number[], target: number): number {
  const n = nums.length;
  // dp[i] represents the maximum number of jumps to reach index i.
  // -1 means the index is unreachable.
  const dp: number[] = new Array(n).fill(-1);

  // Base case: we start at index 0
  dp[0] = 0;

  // Iterate through all possible landing indices
  for (let j = 1; j < n; j++) {
    // Check all possible starting indices to jump to j
    for (let i = 0; i < j; i++) {
